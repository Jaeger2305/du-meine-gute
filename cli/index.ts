...
async function main() {
  ...
  while (player.availableActions.length) {
    // console.clear();
    ...
    const chosenAction = await prompts({
      ...
      name: "handler",
      choices: player.availableActions.map((a) => ({
        title: a.type,
        value: a.handler,
      })),
    });
    ...
    await chosenAction.handler(game, player);
    ...
    if (!player.availableActions.length) {
      const serverResponse = roundSteps[step++ % roundSteps.length](
        game,
        player
      );
