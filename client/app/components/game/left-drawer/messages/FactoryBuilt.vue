<template>
  <BaseMessage v-bind="$props">
    <Label :textWrap="true">
      <FormattedString>
        <Span text="Built factory " />
        <Span text="PLACEHOLDER - no api response data" fontWeight="bold" />
      </FormattedString>
    </Label>
    <template v-slot:icon>
      <Image src="~/assets/images/factories/unknown.png" />
    </template>
  </BaseMessage>
</template>
...
import { FactoryBuildResponse } from "../../../../game/server-action/types";
...
export default {
  ...
  props: {
    ...
    eventDetails: {
      type: Object as PropType<FactoryBuildResponse["response"]>,
      required: true,
    },
    message: {
      type: String,
